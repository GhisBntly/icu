...
public:
    PropertyValueNameGetter(UCharProperty prop) : property(prop) {}
...
void dumpBinaryProperty(UCharProperty uproperty, FILE* f) {
...
void dumpValueEntry(UCharProperty uproperty, int v, bool is_mask, FILE* f) {
...
void dumpEnumeratedProperty(UCharProperty uproperty, FILE* f) {
...
void dumpBidiMirroringGlyph(FILE* f) {
    UCharProperty uproperty = UCHAR_BIDI_MIRRORING_GLYPH;
...
void maybeDumpMaskValue(UCharProperty uproperty, uint32_t v, uint32_t mask, FILE* f) {
...
void dumpGeneralCategoryMask(FILE* f) {
    ...
    UCharProperty uproperty = UCHAR_GENERAL_CATEGORY_MASK;
...
int exportUprops(int argc, char* argv[]) {
    ...
    if (options[OPT_ALL].doesOccur) {
        ...
        while (true) {
            ...
            UCharProperty uprop = static_cast<UProperty>(i);
    ...
    for (const char* propName : propNames) {
        UCharProperty propEnum = u_getPropertyEnum(propName);
